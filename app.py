...
def index():
    ...
    return render_template('index.html', metrics=metrics, column_names=column_names, datetime=datetime)
...
if __name__ == '__main__':
    try:
        conn = get_db_connection()
        conn.close()
        print("\n--- INFO: Connexion de test à la base de données Azure RÉUSSIE ---")
    except Exception as e:
        print("\n!!! ERREUR FATALE: Impossible de se connecter à la base de données !!!")
        print(f"Erreur : {e}\n")
